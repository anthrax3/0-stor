...
class SetupZstorPacket:
    ...
    def stop_data_shards(self,):
        ...
        for hostname, prefab in self._zstordb_devices.items():
            ...
            if self.zstordb_prof:
                ...
                profdir = os.path.join(_PROF_DIR, hostname)
    ...
    def init_benchmark(self, 
        ...
        node = self.p_client.startDevice(hostname=_ZSTORBENCH_HOSTNAME, plan=plan, os=os, facility=facility)
    ...
    profile=None, profile_dir="./zstor_client_profile/"):
        """
        ...
        if profile != None:
            j.tools.prefab.local.core.dir_ensure(profile_dir)
            prefab.core.download(_PROF_DIR, profile_dir)
